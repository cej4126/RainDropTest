...
groupshared uint _maxDepthVS; // tile's maximun depth in view-space.
...
void CullLightsCS(ComputeShaderInput csIn)
{
    ...
    //
    Texture2D<float4> texture = ResourceDescriptorHeap[ShaderParams.DepthBufferSrvIndex];
    const float depth = texture[csIn.DispatchThreadID.xy].r;
    ...
    const float3 pos = UnprojectUV(csIn.DispatchThreadID.xy * invViewDimensions, depth, GlobalData.InverseViewProjection).xyz;
